 ...
 */
router.get('/plans/:chainId', async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
router.get('/pair/:depositCoin/:settleCoin', async (req: Request, res: Response): Promise<void> => {
  try {
    const depositCoin = req.params.depositCoin as string;
    const settleCoin = req.params.settleCoin as string;
 ...
 */
router.post('/create-subscription-shift', async (req: Request, res: Response): Promise<void> => {
  try {
    const userIp = getUserIp(req);
    ...
    if (!plan.active) {
      res.status(400).json({ error: 'Plan is not active' });
      return;
    }
 ...
 * Get shift status
 */
router.get('/shift-status/:id', async (req: Request, res: Response): Promise<void> => {
  try {
